...
DECLARE @msg nvarchar(MAX) = N'Завершение процесса : Загрузка данных и обновление куба' + @crlf + N'Подпроцесс: ' + @sPackage
...
--повторяем цикл запросов пока данные не появятся, ждем 2 мин
IF @iRes is null WAITFOR DELAY '00:00:05'
IF @iRes is null SET @i += -1
IF @iRes is not null SET @i = 0
END

  SET @msg = @msg + @crlf + N'Время начала : ' + ISNULL(FORMAT(@dStart, 'dd.MM.yyyy HH:mm:ss'), N'не определено')
  SET @msg = @msg + @crlf + N'Время завершения : ' + ISNULL(FORMAT(@dEnd, 'dd.MM.yyyy HH:mm:ss'), N'не определено')
  SET @msg = @msg + @crlf + N'Статус выполнения : ' + ISNULL(iif(@iRes = 0, N'Успешно', N'Ошибка'), N'не определено')
...
BEGIN
	...
	SET @msg = N'Текст ошибки : ' + @crlf + @sErrMsg
